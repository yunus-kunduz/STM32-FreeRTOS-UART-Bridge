...
/* USER CODE BEGIN PV */
uint8_t rx_data; // UART'tan gelen tek baytlık veri
extern osMessageQueueId_t DataQueueHandle; // MX tarafından oluşturulan kuyruk ID'si
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart1, &rx_data, 1); // UART1'den 1 bayt veri beklemeye başla
  ...
  DataQueueHandle = osMessageQueueNew (64, sizeof(uint8_t), &DataQueue_attributes);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    // Gelen baytı kuyruğa (Queue) gönder (ISR güvenli versiyonu)
    osMessageQueuePut(DataQueueHandle, &rx_data, 0, 0);

    // Bir sonraki bayt için kesmeyi tekrar aktif et
...
void StartReceiverTask(void *argument)
{
  ...
  for(;;)
  {
   // Kuyrukta veri var mı diye bekle (osWaitForever)
    if (osMessageQueueGet(DataQueueHandle, &received_byte, NULL, osWaitForever) == osOK)
    {
     // Eğer gelen veri varsa PD3 (LED1_RED) yak/sön yap
